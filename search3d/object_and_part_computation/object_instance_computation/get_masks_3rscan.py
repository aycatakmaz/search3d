...
@hydra.main(config_path="conf", config_name="config_base_class_agn_masks_3rscan.yaml")
def get_class_agnostic_masks_3rscan(cfg: DictConfig):
    ...
    for scene_ply_path in scene_ply_paths:
        ...
        if "3rscan" in scene_ply_path:
            batch = list(batch)
            batch[-1] = [scene_ply_path.split("/")[-2]]
            batch = tuple(batch)
...
def main(cfg: DictConfig):
    get_class_agnostic_masks_3rscan(cfg)
